...
public class AccountsApiController implements AccountsApi {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<List<AccountResponseDTO>> getAccounts( @Parameter(in = ParameterIn.QUERY, description = "skips the list of accounts", schema = @Schema()) @Valid @RequestParam(value = "skip") Integer skip,  @Parameter(in = ParameterIn.QUERY, description = "fetch the needed amount of accounts", schema = @Schema()) @Valid @RequestParam(value = "limit") Integer limit) {

        List<Account> accounts = accountService.findAllByAccountIdAfterAndAccountIdBefore(skip,limit);
